...
func recoverTree(root *hard.TreeNode) {
	...
	prev := &hard.TreeNode{Val: math.MinInt32 - 1}
	var err1, err2 *hard.TreeNode

	var inorder func(root *hard.TreeNode)
	inorder = func(root *hard.TreeNode) {
		if root == nil {
			return
		}
		inorder(root.Left)
